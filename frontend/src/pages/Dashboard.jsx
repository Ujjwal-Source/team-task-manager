...
import { CheckSquare, Clock, AlertTriangle, PlayCircle, Folders, TrendingUp } from 'lucide-react';
    ...
    <div>
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card" style={{ borderColor: '#2a2a38' }}>
          ...
          <div className="stat-value" style={{ color: '#f0f0ff' }}>{stats?.myTasks ?? 0}</div>
        </div>
        <div className="stat-card" style={{ borderColor: '#2a2a38' }}>
          <div className="stat-label">📁 Projects</div>
          <div className="stat-value" style={{ color: '#f0f0ff' }}>{projectList.length}</div>
        </div>
      </div>
            ...
            <div>
              {recentTasks.map(t => (
                <div key={t.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                  <div>
                    <div style={{ fontSize: '13px', fontWeight: 500, color: isOverdue(t) ? 'var(--overdue)' : 'var(--text)' }}>{t.title}</div>
                    <div style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '2px' }}>{t.assigneeName || 'Unassigned'}</div>
                  </div>
                  <span className={`badge badge-${t.status === 'in-progress' ? 'inprogress' : t.status}`}>
                    {t.status}
                  </span>
                </div>
              ))}
              ...
              {projectList.map(p => (
                <div key={p.id} style={{ padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--muted)' }}>{p.taskCount} tasks</div>
                  </div>
                  <div style={{ display: 'flex', gap: '12px', marginTop: '6px' }}>
                    <div style={{ flex: 1, height: '4px', background: 'var(--border)', borderRadius: '2px', overflow: 'hidden' }}>
                      <div style={{
                        height: '100%',
                        width: p.taskCount ? `${Math.round(((p.taskCount - p.overdueCount) / p.taskCount) * 100)}%` : '0%',
                  ...
                  <div style={{ fontSize: '11px', color: 'var(--muted)', marginTop: '4px' }}>
                    {p.memberCount} members · {p.overdueCount > 0 ? <span style={{ color: 'var(--overdue)' }}>{p.overdueCount} overdue</span> : 'No overdue'}
                  </div>
                </div>
              ))}
            </div>
